...
frappe.pages['engagement-tracker-r'].on_page_load = function (wrapper) {
	const page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'Engagement Tracker Reboard',
		single_column: true
	});
	...
	function downloadAsExcel(title, records, isReflection = false, isObjective = false) {
		...
		if (isReflection) {
			data = records.map((r, i) => ({
				'#': i + 1, Employee: r.employee_name, Year: r.academic_year, School: r.school_details, Campus: r.campus, Type: r.type, Content: r.content
			}));
		} else if (isObjective) {
			data = records.map((r, i) => ({
				'#': i + 1, Employee: r.employee_name, Year: r.academic_year, School: r.school_details, Campus: r.campus, Category: r.category, Details: r.details
			}));
		} else {
			const keys = getDynamicKeys(records);
			data = records.map((r, i) => {
